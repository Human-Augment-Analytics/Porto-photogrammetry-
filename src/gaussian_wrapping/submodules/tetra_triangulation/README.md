...
If your environment also exports other CUDA libraries through `LD_LIBRARY_PATH` (for example from AliceVision), the extension can fail to import even after a successful build. A common symptom is an error from `libcusparse.so.12` about `__nvJitLinkCreate_12_8`. In that case, remove the conflicting path from `LD_LIBRARY_PATH` or ensure PyTorch's bundled CUDA libraries take precedence at runtime.
