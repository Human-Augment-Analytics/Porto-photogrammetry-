...
def parse_arguments():
    ...
    parser.add_argument(
        "--extract_poses_only",
        action="store_true", 
        help="Extract only camera poses for coordinate system analysis (minimal processing)"
    )
...
def extract_poses_only(predictions, output_dir, input_dir):
    """
    Extract JUST the camera poses for analysis (minimal data).
    ...
    Args:
        ...
        input_dir: Original input directory (for reference)
    """
    print("Extracting camera poses for analysis...")
    
    # Create minimal pose data structure
    pose_data = {
        "extrinsic": predictions["extrinsic"],  # Camera-to-world transforms
        "intrinsic": predictions["intrinsic"],  # Camera intrinsic parameters
        "metadata": {
            "source": "VGGT",
            "input_dir": input_dir,
            "timestamp": time.time(),
            "num_frames": predictions["extrinsic"].shape[0],
            "extraction_purpose": "coordinate_system_analysis"
        }
    }
    
    # Extract camera positions for quick analysis
    extrinsics = predictions["extrinsic"]  # Shape: (num_frames, 4, 4)
    camera_positions = extrinsics[:, :3, 3]  # Extract translation components
    
    pose_data["camera_positions"] = camera_positions
    pose_data["analysis_preview"] = {
        "num_cameras": camera_positions.shape[0],
        "position_range_x": [float(camera_positions[:, 0].min()), float(camera_positions[:, 0].max())],
        "position_range_y": [float(camera_positions[:, 1].min()), float(camera_positions[:, 1].max())], 
        "position_range_z": [float(camera_positions[:, 2].min()), float(camera_positions[:, 2].max())],
        "centroid": [float(camera_positions[:, 0].mean()), 
                    float(camera_positions[:, 1].mean()), 
                    float(camera_positions[:, 2].mean())]
    }
    
    # Save to JSON for easy inspection
    timestamp = int(time.time())
    poses_file = os.path.join(output_dir, f"vggt_poses_{timestamp}.json")
    ...
    pose_data_json = convert_numpy(pose_data)
    
    with open(poses_file, 'w') as f:
        import json
        json.dump(pose_data_json, f, indent=2)
    
    print(f"✅ Saved camera poses to: {poses_file}")
    print(f"📊 Quick analysis:")
    print(f"  Number of camera poses: {pose_data['analysis_preview']['num_cameras']}")
    print(f"  X range: {pose_data['analysis_preview']['position_range_x']}")
    print(f"  Y range: {pose_data['analysis_preview']['position_range_y']}")
    print(f"  Z range: {pose_data['analysis_preview']['position_range_z']}")
    print(f"  Centroid: {pose_data['analysis_preview']['centroid']}")
...
def main():
    ...
    try:
        ...
        if args.extract_poses_only:
            ...
            print("\n" + "="*60)
            print("🔍 Camera poses extracted for analysis!")
            print("="*60)
            print("Run this command to analyze the coordinate system:")
            print()
            print(f"python analyze_vggt_poses.py {poses_file}")
            print()
            print("="*60)
            return  # Exit early, skip GLB and NeuS2 processing
